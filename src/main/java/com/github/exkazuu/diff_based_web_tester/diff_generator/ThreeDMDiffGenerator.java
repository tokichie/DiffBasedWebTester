...
public class ThreeDMDiffGenerator extends HtmlDiffGenerator {
	...
	public String generateDiffContent(String input1, String input2,
		...
		try {
			...
			baseDocument = (BaseNode) parser.parse(TMP_IN_FILE1,
					matching.getBaseNodeFactory());
			branch = (BranchNode) parser.parse(TMP_IN_FILE2,
					matching.getBranchNodeFactory());
			tmpFile1.delete();
			tmpFile2.delete();
		} catch (Exception e) {
		...
		try {
			...
			System.err
					.println("Exception while obtaining diff " + e.toString());
			e.printStackTrace();
			return null;
		}
	}
